...
//         api_key: get_token(_key.claims.id).await.unwrap().unwrap(),
...
//             }else{
//                 let _ = insert_ai_history(_key.claims.id, question.clone(), ans.clone()).await;
//             }
//             ans = ans + &string1;
...
pub async fn processed_stream(
    _key: JWTGuard,
    data: Json<CodeRequest>,
) -> Result<EventStream<impl Stream<Item = Event> + Sized>, &'static str> {
    // 创建处理管道
    let client = Client::new();
    let query_table = QueryTable {
        api_key: get_token(_key.claims.id).await.unwrap().unwrap(),
        messages: match data.code.clone() {
            ...
            Some(code) => vec![
                Message {
                    content: format!("这是一段rust代码 :{}", code),
                    ..Message::default()
                },
                Message {
                    content: data.question.clone(),
    ...
    let user_id = _key.claims.id;
    let question_clone = question.clone();

    // 使用EventStream返回SSE响应
    Ok(EventStream! {
        let mut byte_stream = response.bytes_stream();
        while let Some(item) = byte_stream.next().await {
            ...
            // 处理所有数据块
            for data_block in chunk_str.split("data: ").skip(1) {
                if data_block.contains("[DONE]") {
                    // 流结束时插入历史记录
                    let _ = insert_ai_history(user_id, question_clone.clone(), complete_response.clone()).await;
                    continue;
                }

                // 提取内容
                let content = gjson::get(data_block, "choices.0.delta.content").to_string();
                if !content.is_empty() {
                    // 累积完整响应
